...
describe Enumerable do
  ...
  describe '#my_each' do
    ...
    it 'returns the range of numbers' do
      expect(rg.my_each { |nber| }).to eql(rg)
    end
  ...
  describe '#my_each_with_index' do
    it 'returns the array of numbers' do
      expect(array.my_each_with_index { |n, _i| n }).to eql(array)
    end

    it 'returns the hash' do
      expect(hs.my_each_with_index { |k, _v, _i| k }).to eql(hs)
    end

    it 'returns range of numbers' do
      expect(rg.my_each_with_index { |nbers, _i| nbers }).to eql(rg)
    end
  ...
  describe '#my_select' do
    ...
    it 'returns an array with strings that are true to the given block' do
      expect(%w[f r e d].my_select { |n| n =~ [/sdlfua/] }).to eql([])
    end
  end
  ...
  describe 'my_all?' do
    ...
    it 'returns true if all the elements contain a given regex' do
      expect(%w[how hello common].my_all?(/o/)).to eql(true)
    end
  end
  ...
  describe '#my_any' do
    ...
    it 'returns true if one of the elements contain a given regex' do
      expect(%w[how hello commn].my_any?(/o/)).to eql(true)
    end
  ...
  describe '#my_none' do
    ...
    it 'returns true if none of them contain a given regex' do
      expect(%w[hw hell cmmn].my_none?(/o/)).to eql(true)
    end
  ...
  describe '#my_count' do
    ...
    describe '#my_map' do
      ...
      it 'iterates through a range and returns the modified values' do
        expect(array.my_map { |nber| nber + 1 }).to eql([2, 3, 4, 5, 6])
      end
    end
    ...
    describe '#my_inject' do
      ...
      it 'returns the result of a mathematical expression passed to the argument' do
        expect((5..10).my_inject(20, :+)).to eql(65)
      end
      ...
      it 'returns the the longest string' do
        expect(%w[Mih Frederick ngum].my_inject do |acc, str|
                 acc.length > str.length ? acc : str
               end).to eql('Frederick')
      end
